...
# # anim.save('test.mp4')
#
# anim = animation.FuncAnimation(fig, animate, frames=range(5000), interval=1000/120, blit=False, repeat=False, fargs=(xy, vel, force, rho, pre, h, mass, k, density, f_cons, dt, visc))
#
# writer = PillowWriter(fps=120)  # fps 값을 조정할 수 있습니다
# anim.save('test_120.gif', writer=writer)  # 저장할 파일의 확장자를 .gif로 변경
#
# plt.show()
...
scene = canvas()
particles = [sphere(pos=vector(xy[i, 0], xy[i, 1], 0), radius=3) for i in range(n)]

# scene = canvas()
# particles = [sphere(pos=vector(xy[i, 0], xy[i, 1], 0), radius=1) for i in range(n)]
...
while True:
    ...
    for i, particle in enumerate(particles):
        particle.pos = vector(xy[i, 0], xy[i, 1], 0)